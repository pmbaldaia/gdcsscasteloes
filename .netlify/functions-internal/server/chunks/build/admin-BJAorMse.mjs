...
import { u as useHead, f as useThemeMode, a as useRoute, e as useAuth } from './server.mjs';
import { _ as _sfc_main$5 } from './ThemeToggle-ByhJnfua.mjs';
import { u as useToast } from './useToast-BJDEtn_Q.mjs';
import { u as useConfirm } from './useConfirm-CFUPPoka.mjs';
import { _ as _sfc_main$4 } from './admin-DFCZIDn5.mjs';
...
const allGroups = [
  ...
  { label: "Conte\xFAdo", items: [
    { label: "P\xE1ginas", icon: "lucide:file-text", to: "/admin/paginas" },
    { label: "Blocos de conte\xFAdo", icon: "lucide:blocks", to: "/admin/blocos" },
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const roleLabel = computed(
      () => {
        var _a;
        return { admin: "Administrador", viewer: "Consulta e edi\xE7\xE3o" }[((_a = user.value) == null ? void 0 : _a.role) || ""] || "Utilizador";
      }
    );
    const pageMeta = computed(() => {
      ...
      const map = [
        ...
        { test: (p) => p.startsWith("/admin/paginas"), label: "P\xE1ginas", icon: "lucide:file-text" },
        { test: (p) => p.startsWith("/admin/blocos"), label: "Blocos de conte\xFAdo", icon: "lucide:blocks" },
        { test: (p) => p.startsWith("/admin/menus"), label: "Menus", icon: "lucide:menu" },
        { test: (p) => p.startsWith("/admin/perfil"), label: "O meu perfil", icon: "lucide:user-cog" },
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<button class="user-menu" type="button"><span class="user-menu__avatar">`);
      if ((_a = unref(user)) == null ? void 0 : _a.avatar) {
        _push(`<img${ssrRenderAttr("src", unref(user).avatar)}${ssrRenderAttr("alt", unref(user).name || "Perfil")} class="user-menu__avatar-image">`);
      } else {
        _push(`<!--[-->${ssrInterpolate((((_b = unref(user)) == null ? void 0 : _b.name) || "AD").slice(0, 2).toUpperCase())}<!--]-->`);
      }
      _push(`</span><span class="user-menu__copy"><strong>${ssrInterpolate(((_c = unref(user)) == null ? void 0 : _c.name) || "Administrador")}</strong><small>${ssrInterpolate(unref(roleLabel))}</small></span>`);
      _push(ssrRenderComponent(_component_Icon, {
      ...
      if (unref(menuOpen)) {
        ...
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(ssrRenderComponent(_component_Icon, { name: "lucide:user-cog" }, null, _parent2, _scopeId));
              _push2(` O meu perfil `);
            } else {
              return [
                createVNode(_component_Icon, { name: "lucide:user-cog" }),
                createTextVNode(" O meu perfil ")
              ];
            }
          }),
          _: 1
...
//# sourceMappingURL=admin-BJAorMse.mjs.map
