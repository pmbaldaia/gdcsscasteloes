...
export default defineEventHandler(async(event)=>{
 ...
 if(path[0]==='media'){
   ...
   if(method==='DELETE'&&path[1]){
     requireAdmin(user)
     if(await mediaService.remove(decodeURIComponent(path[1]))){setResponseStatus(event,204);return null}
     fail(404,'Ficheiro não encontrado')
   }
 }
 ...
 if(method==='GET'&&id){
   ...
   if(!row)fail(404,'Registo não encontrado')
   return row
 }
 ...
 if(method==='DELETE'&&id){
   requireAdmin(user)
   if(await service.remove(id)){setResponseStatus(event,204);return null}
   fail(404,'Registo não encontrado')
 }
 fail(405,'Método não permitido')
})
