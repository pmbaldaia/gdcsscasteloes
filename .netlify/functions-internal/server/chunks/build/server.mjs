...
const __nuxt_page_meta$e = { layout: "blank" };
...
const useAuth = () => {
  ...
  async function logout() {
    try {
      if (token.value) await $fetch(`/api/auth/logout`, { method: "POST", headers: authHeaders() });
    } catch {
    }
    token.value = null;
    user.value = null;
...
const admin_45auth_45global = /* @__PURE__ */ defineNuxtRouteMiddleware(async (to) => {
  ...
  try {
    const user = ([__temp, __restore] = executeAsync(() => me()), __temp = await __temp, __restore(), __temp);
...
const trailing_45slash_45global = /* @__PURE__ */ defineNuxtRouteMiddleware((to) => {
...
const useThemeMode = () => {
  const cookie = useCookie("gdcss_theme", {
    default: () => "light",
    sameSite: "lax",
    maxAge: 60 * 60 * 24 * 365
  });
