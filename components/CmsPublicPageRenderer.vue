...
<template>
  <LandingContainer>
    <template v-for="block in normalBlocks" :key="block.id">
      ...
      <section v-else-if="block.type==='cta'" class="card-surface px-6 py-16 mt-20 mx-auto max-w-5xl flex flex-col items-center text-center"><h2 class="text-neutral-900 text-fluid-4xl font-bold">{{block.title}}</h2><p class="text-neutral-600 mt-4 text-fluid-lg max-w-2xl">{{block.content}}</p><div v-if="block.items?.length" class="mt-6 flex flex-col sm:flex-row gap-3 justify-center items-center"><NuxtLink v-for="(raw,index) in block.items" :key="`${block.id}-${index}`" :to="parseAction(raw,index).url" :class="parseAction(raw,index).style==='secondary'?'btn-club-red w-full sm:w-auto':'btn-club-green w-full sm:w-auto'">{{parseAction(raw,index).label}}</NuxtLink></div></section>
    </template>
